...
export default function OrgByProject({ employees, projects }: Props) {
  ...
  const projectsWithCount = useMemo(() =>
    projects.map(p => ({
      ...p,
      count: employees.filter(e => e.projectIds.includes(p.id)).length,
    })).sort((a, b) => b.count - a.count),
  [projects, employees]);

  const filteredProjects = search
    ? projectsWithCount.filter(p => p.name.toLowerCase().includes(search.toLowerCase()) || p.code.toLowerCase().includes(search.toLowerCase()))
    : projectsWithCount;
          ...
          {filteredProjects.map(p => (
            ...
            >
              <div className="flex-1 min-w-0">
                ...
                <p className={`text-xs font-medium truncate ${selectedProjectId === p.id ? 'text-blue-700' : 'text-slate-700'}`}>{p.name}</p>
                {p.location && <p className="text-[10px] text-slate-400 truncate flex items-center gap-0.5 mt-0.5"><MapPin size={9} />{p.location}</p>}
              </div>
      ...
      <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
        {!selectedProject ? (
          ...
          <>
              ...
              <div className="flex items-start justify-between">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <span className={`text-xs px-2 py-0.5 rounded font-medium ${TYPE_COLORS[selectedProject.type]}`}>{selectedProject.type}</span>
                    <span className="text-xs text-slate-400 font-mono">{selectedProject.code}</span>
                    {selectedProject.location && (
                      <span className="text-xs text-slate-400 flex items-center gap-0.5"><MapPin size={10} />{selectedProject.location}</span>
                    )}
                  </div>
                ...
                <div className="flex items-center gap-3 text-right">
                  <div className="text-right">
                    <p className="text-2xl font-bold text-slate-800">{projectEmployees.length}</p>
            ...
            {/* Employee cards grouped by department */}
