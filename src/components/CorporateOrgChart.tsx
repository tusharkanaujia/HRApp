...
.corp-org {
  ...
  overflow: auto;            /* horizontal + vertical scroll for the full-size chart */
  padding: 20px;
}
...
.corp-org .page {
  ...
  /* Fixed width set by the Width control. The page keeps full-size cards and
     can exceed the viewport — the user scrolls horizontally to see it. */
    ...
    <div className="relative w-full h-full overflow-hidden">
      <div
        ...
        style={{
          ...
          // out instead of cramming. The chart scrolls horizontally when wider
          // than the viewport.
          ...(width ? {
      ...
      {editMode && widthOpen && (
        <div className="absolute top-14 right-3 z-40 bg-white rounded-xl shadow-lg border border-slate-200 p-3 w-64 text-xs text-slate-700" onClick={e => e.stopPropagation()}>
          ...
          <p className="text-[11px] text-slate-400 mb-2 leading-relaxed">
            Set how wide the chart is. Cards stay full size — drag the slider right to spread the columns out, then scroll sideways to see it all. Drag left to fit a smaller screen.
          </p>
          <label className="block text-slate-500 mb-1">
            Layout width · {width ? `${width}px` : `Default (${DEFAULT_PAGE_W}px)`}
          </label>
          <input
            type="range"
            min={MIN_PAGE_W}
